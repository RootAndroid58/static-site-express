(function (port) {
  ...
  var client = new faunadb.Client({ secret: site.db })
  ...
  // Middlewares.
  // GET favicon.ico
  app.use('/', favicon(path.join('public','assets' ,'images' , 'favicon.ico')))

  // to serve the static files from the /public folder
  app.use('/', express.static(path.join('public')))
  app.get('/message-sent', function (req, res) {
    res.writeHead(200, { 'Content-Type': 'text/html' })
    fs.readFile(path.join('public', 'message-sent.html'), { encoding: 'utf8' }, (err, data) => {
      if (err) throw err
      res.end(data)
    }) 
  })
